...
Layer 2: Redis (shared across workers, ~3ms)
Layer 3: Upstream HTTP fetch via proxy (~200ms)
...
from app.services.upstream_proxy import fetch_all_agents

logger = logging.getLogger(__name__)

# -- Tuning constants --
MEMORY_FRESH_TTL = 300  # 5 minutes — data considered "fresh"
MEMORY_MAX_STALE_TTL = 3600  # 1 hour — evict after this
...
async def _redis_put(key: str, data: dict) -> None:
    try:
        ...
        await client.set(key, serialized, ex=REDIS_TTL)
    except Exception:
...
async def swr_fetch(
    ...
    Pagination is pass-through: page/limit are included in the cache key
    and forwarded to upstream. Each page is a separate cache entry.
    """
