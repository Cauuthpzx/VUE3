...
from app.services.swr_cache import get_cache_stats, swr_fetch
...
@router.post(
    "/rebate-init",
    summary="Rebate init data",
    description="Get lottery series + first series' games for rebate page.",
)
...
@router.post(
    "/rebate-games",
    summary="Rebate games by series",
    description="Get lottery games for a specific series.",
)
...
@router.post(
    "/rebate",
    summary="Rebate odds panel",
    description="Get rebate odds panel for a specific lottery game.",
)
async def proxy_rebate(
    ...
    series_id = params.get("series_id", "")
    return await fetch_rebate_panel(
        db, lottery_id, series_id, _extract_agent_id(params)
    )
...
@router.get(
    "/cache-stats",
    summary="SWR cache statistics",
    description="Returns memory cache entry count and limits.",
)
...
@router.post(
    "/{endpoint}",
    summary="Proxy upstream data",
    description="Proxy request with 3-layer SWR cache (Memory -> Redis -> Upstream). "
    "Use ?_fresh=1 to bypass cache.",
)
async def proxy_upstream(
    ...
    # Report endpoints: compute column totals from full dataset
