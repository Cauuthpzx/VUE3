...
class Bet(Base):
    ...
    status: Mapped[int] = mapped_column(Integer, default=-9)
...
class BetThirdParty(Base):
    ...
    turnover: Mapped[float] = mapped_column(Numeric(16, 2), default=0)
    prize: Mapped[float] = mapped_column(Numeric(16, 2), default=0)
    win_lose: Mapped[float] = mapped_column(Numeric(16, 2), default=0)
    bet_time: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    synced_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=lambda: datetime.now(UTC)
    )
    created_at: Mapped[datetime] = mapped_column(
